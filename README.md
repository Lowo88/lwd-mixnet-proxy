...
> **Status: early.** The core mechanism is implemented and unit-tested, and its measurement against
> the live mixnet has not been run yet. Until it has, treat this as unproven.
...
| `--probe-concurrency` | Streams opened at once. One retries in series and pays a deadline per failure; more opens several and takes the first to answer, at the cost of reply blocks. |
...
```
lwd-mixnet-bench --server <address> --trials 300 --attempts 4 --concurrency 1
```

What the run has to show, and why a single threshold would not have been enough, is in
[ADR 0005](docs/decisions/0005-what-the-measurement-has-to-show.md).
